...
export const POST: RequestHandler = async ({ request, cookies }) => {
	...
	const existing = db.select().from(students).where(eq(students.name, name.trim())).get();
	if (existing) {
	...
	db.insert(students).values({ id, name: name.trim(), created_at: now, last_active: now }).run();
...
export const PUT: RequestHandler = async ({ request, cookies }) => {
	...
	db.update(students).set({ last_active: new Date().toISOString() }).where(eq(students.id, id)).run();
	cookies.set('student_id', id, { path: '/', httpOnly: true, sameSite: 'lax', maxAge: 60 * 60 * 24 * 365 });
