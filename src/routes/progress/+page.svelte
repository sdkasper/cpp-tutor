...
<svelte:head>
	<title>{$t('progress.title')} | C++ Tutor</title>
</svelte:head>

<div class="mx-auto max-w-3xl px-4 py-8 page-fade-in">
	<h1 class="text-3xl font-bold mb-8" style="color: var(--color-text);">{$t('progress.title')}</h1>

	<!-- Stats overview -->
	<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
		<div class="card p-5 text-center">
			<p class="text-3xl font-bold text-green-400">{data.stats.completed}</p>
			<p class="text-sm text-slate-400 mt-1">{$t('progress.solved')}</p>
		</div>
		<div class="card p-5 text-center">
			<p class="text-3xl font-bold text-yellow-400">{data.stats.inProgress}</p>
			<p class="text-sm text-slate-400 mt-1">{$t('progress.inProgress')}</p>
		</div>
		<div class="card p-5 text-center">
			<p class="text-3xl font-bold text-slate-300">{data.stats.total}</p>
			<p class="text-sm text-slate-400 mt-1">{$t('progress.totalProblems')}</p>
		</div>
	</div>
	...
	<div class="mb-8">
		<div class="flex justify-between text-sm mb-2">
			<span class="text-slate-400">{$t('progress.overall')}</span>
	...
	{#if data.problems.length === 0}
		<div class="card p-12 text-center">
			...
			<p class="text-slate-400 text-lg mb-2">{$t('progress.noProgress')}</p>
			<p class="text-slate-500 text-sm">{$t('progress.noProgressHint')}</p>
		</div>
	{:else}
		<div class="space-y-3">
			{#each data.problems as problem}
				...
				<a href="/tutor/{problem.id}" class="flex items-center justify-between card card-hover px-4 py-3 hover:border-indigo-500/50 transition-all">
					...
					{#if problem.progress}
						<span class="text-xs text-slate-500">{problem.progress.hints_used} {$t('progress.hintsUsed')}</span>
					{/if}
				</a>
			{/each}
		</div>
